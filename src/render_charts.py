"""Render the page and the matrix across the K6 width ladder.
...
pair. Adapted from cascadia-revenue-assurance/src/render_charts.py; the one
chart here is an HTML table, so there is no ECharts grid to read and instead
the table's own scroll state is recorded at every width.

320 px is Rule 5.3 / WCAG 1.4.10 reflow, the floor the system claims to
support. The page is opened as a file:// URL and makes no network request.

Checked at every width, and each is a FAIL that exits non-zero:
  * horizontal overflow of the DOCUMENT (Rule 5.3's 1.4.10 clause); the table
    may scroll inside its wrapper, the page may not
  * the rendered provenance strip's segment count (K5): exactly 3
  * any page error or console error

Output: docs/renders/page-<width>.png, matrix-<width>.png, and
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        for width in widths:
            ...
            for cid in CHARTS:
                page.locator("#%s" % cid).screenshot(path=str(out_dir / ("%s-%d.png" % (cid, width))))
            seg = page.evaluate(
                """() => Array.from(document.querySelectorAll('.cascadia-provenance'))
                        .map(n => n.textContent.split(' \\u00b7 ').length)""")
            if len(seg) != len(CHARTS):
                segments.append((width, "strip count", len(seg)))
            for cid, n in zip(CHARTS, seg):
                if n != EXPECTED_SEGMENTS:
                    segments.append((width, cid, n))
            state = page.evaluate(
                """() => { const h = document.getElementById('matrix');
                          const s = h.querySelector('.table-scroll');
                          const t = h.querySelector('table.matrix');
                          return { host: h.clientWidth, narrow: h.classList.contains('narrow'),
                                   table: t.scrollWidth, wrapper: s.clientWidth,
                                   scrolls: s.scrollWidth > s.clientWidth + 1 }; }""")
            state_log[str(width)] = state
            over = page.evaluate(
                """() => ({scroll: document.documentElement.scrollWidth,
                          client: document.documentElement.clientWidth})""")
            print("  width %d: strips %s  host %d px narrow=%s table %d px in %d px wrapper, scrolls=%s"
                  % (width, seg, state["host"], state["narrow"], state["table"], state["wrapper"], state["scrolls"]))
    ...
    (out_dir / "k6-ladder.json").write_text(json.dumps({
        ...
        "matrix_state_by_width": state_log,
